...
--PRAGMA SERIALLY_REUSABLE;
  ...
  *                                                                              *
  *
  * Oracle 19c / AL32UTF8 compatibility patch
  * Public compatibility edition: 2026-08-26
  *
  * This source derives from a legacy PL_FPDF deployment whose public
  * PL_FPDF_VERSION constant is 0.9.4. The historical source header below
  * identifies an earlier 0.9.2 lineage; both facts are retained to avoid
  * misrepresenting the source history.
  ********************************************************************************
  * Cette librairie PL/SQL est un portage de la version 1.53 de FPDF, celebre    *
  * classe PHP developpee par Olivier PLATHEY (http://www.fpdf.org/)             *
  ********************************************************************************
  ...
  ********************************************************************************/
  ...
  function p_escape(pstr varchar2) return varchar2;
  ...
  procedure SetFontFx(pfamily varchar2,pstyle varchar2 default '',psize number default 0);
  ...
  function return_output_blob(pname varchar2 default null) return blob;
...
procedure  resetWpdf;
  ...
  function getImageFromUrl(p_Url varchar2) return blob;
  ...
  -- aggiunte 01/12/2011
  procedure Ellipse(x in number, y in number, rx in number, ry in number, style in char default 'D');
  procedure Circle(x in number, y in number, r in number, style in char default 'D');
  procedure Arc(x1 in number, y1 in number, x2 in number, y2 in number, x3 in number, y3 in number);
  procedure RoundedRect(x in number,
                        y in number,
                        w in number,
                        h in number,
                        rTopLeft in Number,
                        rTopRight in Number,
                        rBottomRight in Number,
                        rBottomLeft in Number,
                        style in char default '');
  procedure MultiCellWithBullet(w in number,
                                  h in number,
                                  blt in varchar2,
                                  blt_margin in number,
                                  txt in varchar2,
                                  border in varchar2 default 0,
                                  align in varchar2 default 'J',
                                  fill in number default 0);
  procedure TextWithDirection(x in number, y in number, p_txt in varchar2, p_direction in varchar2 default 'R');
  procedure TextWithRotation(x in number, y in number, p_txt in varchar2, p_textAngle in number, p_fontAngle in number default 0);
  procedure ClippingRect(px in number, py in number, pw in number, ph in number, p_outline in boolean default false);                       
  procedure UnsetClipping;
  function getImageFromDatabase(pFile varchar2) return blob;
...
--PRAGMA SERIALLY_REUSABLE;
  /*******************************************************************************
* Logiciel : PL_FPDF                                                           *
* Version :  0.9.4 + Oracle 19c compatibility patch                              *
* Public compatibility edition: 2026-08-26                                      *
* Date :     13/06/2006                                                        *
* Auteur :   Pierre-Gilles Levallois                                           *
* Licence :  GPL                                                               *
...
-- AL32UTF8-safe key for font metric associative arrays. A logical single
-- character can require up to 4 bytes in AL32UTF8.
subtype char_key is varchar2(4 char);
...
type charSet is table of pls_integer index by char_key;
...
type recImage is record ( n number,  	 	  	 		-- indice d'insertion dans le document
						  ...
						  pal blob,					-- colors palette informations (binary-safe) 
...
--- END AG MOD
 ...
 -- Proprietes ajoutees lors du portage en PLSQL.
 ...
 -- A verifier au court de la validation du portage.
 originalsize word;
 size1 word;
 size2 word;
/*******************************************************************************
...
*******************************************************************************/
----------------------------------------------------------------------------------
-- proc. and func. specifiques ajoutees au portage.
----------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------
-- Inclusion des metriques d'une font.
----------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------
-- p_getFontMetrics : recuperer les metric d'une font.
----------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------
-- Parcours le tableau des images et renvoie true si l'image cherche existe 
-- dans le tableau.
----------------------------------------------------------------------------------
function imageExists(pFile varchar2) return boolean is
begin
  if (images.exists(lower(pFile))) or (images.exists(pFile)) then
     return true;
  end if;
  return false;
exception 
...
--------------------------------------------------------------------------------
-- Get an image BLOB from an HTTP/HTTPS URL.
-- Oracle 19c patch: no Oracle Multimedia / Oracle Multimedia dependency.
-- The original binary format is preserved (PNG or JPEG).
--------------------------------------------------------------------------------
function getImageFromUrl(p_Url varchar2) return blob is
  l_blob   blob;
  lv_url   varchar2(2000) := p_Url;
  urityp   URIType;
begin
  -- Preserve legacy handling of relative URLs.
  if instr(lower(lv_url), 'http://') != 1
     and instr(lower(lv_url), 'https://') != 1 then
    lv_url := 'http://' || owa_util.get_cgi_env('SERVER_NAME') || '/' || ltrim(lv_url, '/');
  end if;

  urityp := URIFactory.getURI(lv_url);
  l_blob := urityp.getBlob();

  if l_blob is null or dbms_lob.getlength(l_blob) = 0 then
    Error('pl_fpdf.getImageFromUrl : empty image, image :' || p_Url);
  end if;
...
--------------------------------------------------------------------------------
-- Get an image BLOB from an Oracle table.
-- Legacy function was only a stub and did not actually retrieve data.
-- Kept as a BLOB-returning stub to remove the Oracle Multimedia dependency.
--------------------------------------------------------------------------------
function getImageFromDatabase(pFile varchar2) return blob is
  l_blob blob;
begin
  return l_blob;
...
----------------------------------------------------------------------------------
-- Compatibilite PHP -> PLSQL : proc. and func. specifiques au portages
-- 				 	 		  	ajoutee pour des facilites de traduction
----------------------------------------------------------------------------------
...
begin
  temp := to_number(pstr);
  -- Si on passe la c'est que la variable contient un nombre sinon => exception.
  return false;
exception
when others then
  return true;
...
begin
    -- Procedure inutile avec le PLSQL : toutes les fonts sont chargees en memoire.
	return null;
end p_getfontpath;
...
----------------------------------------------------------------------------------------
-- Oracle 19c / AL32UTF8 compatibility.
-- PDF core fonts use /WinAnsiEncoding: keep the application text in the database
-- character set and convert it to WE8MSWIN1252 only as RAW bytes.
function p_winansi_raw(pstr varchar2) return raw is
  l_raw       raw(32767);
  l_roundtrip varchar2(32767);
begin
  if pstr is null then
    return null;
  end if;

  l_raw := utl_i18n.string_to_raw(pstr, 'WE8MSWIN1252');

  -- Do not silently replace Unicode characters which are not available in
  -- WinAnsi/core PDF fonts.
  l_roundtrip := utl_i18n.raw_to_char(l_raw, 'WE8MSWIN1252');
  if l_roundtrip <> pstr then
    Error('Character not supported by WinAnsi/core PDF fonts: ' || pstr);
  end if;
...
exception
  when others then
    ...
    Error('p_winansi_raw :' || sqlerrm);
    return null;
end p_winansi_raw;

----------------------------------------------------------------------------------------
-- Produce an ASCII-only PDF hexadecimal string containing WinAnsi bytes.
-- Example: Euro (U+20AC) becomes <80>.  Because the PDF syntax itself contains
-- only ASCII hex digits, later AL32UTF8 CLOB/VARCHAR2 -> BLOB conversion cannot
-- corrupt the single-byte text expected by the core fonts.
function p_pdf_text(pstr varchar2) return varchar2 is
  l_raw raw(32767);
begin
  if pstr is null then
    return '<>';
  ...
  l_raw := p_winansi_raw(pstr);
  return '<' || rawtohex(l_raw) || '>';
end p_pdf_text;

----------------------------------------------------------------------------------------
-- The legacy font width tables are indexed by byte positions 0..255. Convert
-- one database character to its WinAnsi byte and then to the corresponding
-- ISO-8859-1 control/codepoint solely for associative-array lookup.
function p_winansi_metric_char(pchar varchar2) return char_key is
  l_raw raw(32767);
begin
  ...
  l_raw := p_winansi_raw(pchar);
  if utl_raw.length(l_raw) <> 1 then
    Error('Invalid WinAnsi character width lookup: ' || pchar);
  end if;

  return utl_i18n.raw_to_char(l_raw, 'WE8ISO8859P1');
end p_winansi_metric_char;

----------------------------------------------------------------------------------------
function p_char_width(pchar varchar2, pwidths charSet) return number is
  l_key char_key;
begin
  if pchar is null then
    return 0;
  ...
  l_key := p_winansi_metric_char(pchar);
  if pwidths.exists(l_key) then
    return pwidths(l_key);
  end if;
...
----------------------------------------------------------------------------------------
-- Binary-safe image stream writer for AL32UTF8 databases.
-- Encodes BLOB bytes as ASCII hex and terminates the PDF ASCIIHex stream with '>'.
----------------------------------------------------------------------------------------
procedure p_putstream_hex(pData in out NOCOPY blob) is
  l_offset         integer := 1;
  l_content_length number := dbms_lob.getlength(pData);
  l_amount         integer;
  l_chunk_size     constant integer := 12000; -- RAWTOHEX => max 24000 chars
  l_buf            raw(12000);
begin
  p_out('stream');

  while l_offset <= l_content_length loop
    l_amount := least(l_chunk_size, l_content_length - l_offset + 1);
    dbms_lob.read(pData, l_amount, l_offset, l_buf);
    p_out(rawtohex(l_buf), false);
    l_offset := l_offset + l_amount;
  end loop;

  -- ASCIIHexDecode end-of-data marker. The following LF is not part of /Length.
  p_out('>', false);
  p_out(chr(10), false);
  p_out('endstream');
exception
  when others then
    error('p_putstream_hex : ' || sqlerrm);
end p_putstream_hex;
  ...
  while (v is not null) loop
    ...
    if(info.cs = 'Indexed') then
      p_out('/ColorSpace [/Indexed /DeviceRGB ' ||
            to_char(dbms_lob.getlength(info.pal) / 3 - 1) || ' ' ||
            to_char(n + 1) || ' 0 R]');
    else
    ...
    -- Image bytes are ASCIIHex encoded before being put into the legacy
    -- VARCHAR2 PDF buffer. The original PNG/JPEG PDF filter is applied after
    -- ASCIIHexDecode by the PDF reader.
    ...
    if(info.parms is not null) then
      -- When ASCIIHexDecode is prepended to the original image filter,
      -- DecodeParms must be an array aligned with the /Filter array.
      -- ASCIIHexDecode has no parameters; PNG Predictor parameters belong
      -- to the second filter (FlateDecode).
      if(info.f is not null) then
        p_out('/DecodeParms [null ' ||
              replace(info.parms, '/DecodeParms ', '') || ']');
      else
        p_out(info.parms);
      end if;
    end if;
    ...
    l_hex_length := (dbms_lob.getlength(info.data) * 2) + 1; -- + '>' terminator
    p_out('/Length ' || l_hex_length || '>>');
    p_putstream_hex(info.data);
    if dbms_lob.istemporary(info.data) = 1 then
      dbms_lob.freetemporary(info.data);
    end if;
    images(v).data := null;
    p_out('endobj');

    -- Indexed PNG palette is binary too: encode it as ASCIIHex as well.
    if(info.cs = 'Indexed') then
      p_newobj();
      l_hex_length := (dbms_lob.getlength(info.pal) * 2) + 1;
      p_out('<</Filter /ASCIIHexDecode /Length ' || l_hex_length || '>>');
      p_putstream_hex(info.pal);
      if dbms_lob.istemporary(info.pal) = 1 then
        dbms_lob.freetemporary(info.pal);
      end if;
      p_out('endobj');
    end if;
...
begin
   -- Replace number of pages
	 if not empty(AliasNbPages) then
		   for i in 1..nb loop
		      ...
		      -- Text strings are encoded as WinAnsi hexadecimal strings on AL32UTF8.
		      -- Replace the hexadecimal form of the alias as well.
		      pages(i) := str_replace(
		                    rawtohex(p_winansi_raw(AliasNbPages)),
		                    rawtohex(p_winansi_raw(to_char(nb))),
		                    pages(i));
		   end loop;
	 end if; 
   ...
   for i in 1..nb loop
      ...
      p_newobj();
      dbms_output.put_line('Page='||i||'   n='||n);
		  p_out('<</Type /Page');
		  p_out('/Parent 1 0 R');
...
--------------------------------------------------------------------------------
-- Parse an image (Oracle 19c native BLOB patch)
-- Supported formats: PNG (color types 0, 2, 3; bit depth <= 8; non-interlaced)
--                    JPEG (baseline/progressive SOF markers supported)
-- GIF/BMP are no longer converted automatically because Oracle Multimedia is not used.
--------------------------------------------------------------------------------
function p_parseImage(pFile varchar2) return recImage is
  ...
  ------------------------------------------------------------------------------
  function raw_u8(p_raw raw, p_pos pls_integer default 1) return pls_integer is
    l_one raw(1);
  begin
    l_one := utl_raw.substr(p_raw, p_pos, 1);
    return utl_raw.cast_to_binary_integer(
             utl_raw.concat(hextoraw('000000'), l_one),
             utl_raw.big_endian
           );
  end raw_u8;

  function raw_u16(p_raw raw) return pls_integer is
  begin
    return utl_raw.cast_to_binary_integer(
             utl_raw.concat(hextoraw('0000'), p_raw),
             utl_raw.big_endian
           );
  end raw_u16;

  function raw_u32(p_raw raw) return number is
  begin
    return utl_raw.cast_to_binary_integer(p_raw, utl_raw.big_endian);
  end raw_u32;

  ------------------------------------------------------------------------------
  -- Parse PNG and build the FlateDecode payload expected by the existing
  -- p_putimages implementation.
  ------------------------------------------------------------------------------
  function parse_png(p_blob blob) return recImage is
    l_info          recImage;
    l_pos           pls_integer := 9; -- after 8-byte PNG signature
    l_blob_len      pls_integer := dbms_lob.getlength(p_blob);
    l_chunk_len     pls_integer;
    l_chunk_type    varchar2(4);
    l_hdr           raw(13);
    l_bit_depth     pls_integer;
    l_color_type    pls_integer;
    l_compression   pls_integer;
    l_filter        pls_integer;
    l_interlace     pls_integer;
    l_colors        pls_integer;
    l_trans_raw     raw(32767);
    l_zero_index    pls_integer;
    l_b             pls_integer;
  begin
    dbms_lob.createtemporary(l_info.data, true);

    while l_pos + 11 <= l_blob_len loop
      l_chunk_len := raw_u32(dbms_lob.substr(p_blob, 4, l_pos));
      l_pos := l_pos + 4;

      l_chunk_type := utl_raw.cast_to_varchar2(dbms_lob.substr(p_blob, 4, l_pos));
      l_pos := l_pos + 4;

      if l_chunk_len < 0 or l_pos + l_chunk_len + 4 - 1 > l_blob_len then
        Error('Incorrect PNG chunk length: ' || pFile);
      end if;

      if l_chunk_type = 'IHDR' then
        if l_chunk_len != 13 then
          Error('Incorrect PNG IHDR length: ' || pFile);
        end if;

        l_hdr := dbms_lob.substr(p_blob, 13, l_pos);
        l_info.w := raw_u32(utl_raw.substr(l_hdr, 1, 4));
        l_info.h := raw_u32(utl_raw.substr(l_hdr, 5, 4));
        l_bit_depth  := raw_u8(l_hdr, 9);
        l_color_type := raw_u8(l_hdr, 10);
        l_compression := raw_u8(l_hdr, 11);
        l_filter      := raw_u8(l_hdr, 12);
        l_interlace   := raw_u8(l_hdr, 13);

        l_info.bpc := l_bit_depth;
        if l_bit_depth > 8 then
          Error('16-bit depth not supported: ' || pFile);
        end if;

        if l_color_type = 0 then
          l_info.cs := 'DeviceGray';
          l_colors := 1;
        elsif l_color_type = 2 then
          l_info.cs := 'DeviceRGB';
          l_colors := 3;
        elsif l_color_type = 3 then
          l_info.cs := 'Indexed';
          l_colors := 1;
        else
          Error('PNG alpha channel/color type not supported: ' || pFile);
        end if;

        if l_compression != 0 then
          Error('Unknown PNG compression method: ' || pFile);
        end if;
        if l_filter != 0 then
          Error('Unknown PNG filter method: ' || pFile);
        end if;
        if l_interlace != 0 then
          Error('Interlaced PNG not supported: ' || pFile);
        end if;

        l_info.parms := '/DecodeParms <</Predictor 15 /Colors ' ||
                        to_char(l_colors) || ' /BitsPerComponent ' ||
                        l_info.bpc || ' /Columns ' || l_info.w || '>>';

      elsif l_chunk_type = 'PLTE' then
        if l_info.pal is not null and dbms_lob.istemporary(l_info.pal) = 1 then
          dbms_lob.freetemporary(l_info.pal);
        end if;
        dbms_lob.createtemporary(l_info.pal, true);
        if l_chunk_len > 0 then
          dbms_lob.copy(l_info.pal, p_blob, l_chunk_len, 1, l_pos);
        end if;

      elsif l_chunk_type = 'tRNS' then
        if l_chunk_len > 32767 then
          Error('PNG transparency chunk too large: ' || pFile);
        end if;

        if l_chunk_len > 0 then
          l_trans_raw := dbms_lob.substr(p_blob, l_chunk_len, l_pos);

          if l_color_type = 0 and l_chunk_len >= 2 then
            l_info.trns(1) := raw_u16(utl_raw.substr(l_trans_raw, 1, 2));
          elsif l_color_type = 2 and l_chunk_len >= 6 then
            l_info.trns(1) := raw_u16(utl_raw.substr(l_trans_raw, 1, 2));
            l_info.trns(2) := raw_u16(utl_raw.substr(l_trans_raw, 3, 2));
            l_info.trns(3) := raw_u16(utl_raw.substr(l_trans_raw, 5, 2));
          elsif l_color_type = 3 then
            -- FPDF 1.53 supports one fully transparent palette index.
            l_zero_index := null;
            for i in 1 .. l_chunk_len loop
              l_b := raw_u8(l_trans_raw, i);
              if l_b = 0 then
                l_zero_index := i - 1; -- PNG palette indexes are zero-based
                exit;
              end if;
            end loop;
            if l_zero_index is not null then
              l_info.trns(1) := l_zero_index;
            end if;
          end if;
        end if;

      elsif l_chunk_type = 'IDAT' then
        if l_chunk_len > 0 then
          dbms_lob.copy(
            l_info.data,
            p_blob,
            l_chunk_len,
            dbms_lob.getlength(l_info.data) + 1,
            l_pos
          );
        end if;

      elsif l_chunk_type = 'IEND' then
        exit;
      end if;

      -- Skip chunk data and CRC.
      l_pos := l_pos + l_chunk_len + 4;
    end loop;

    if l_info.w is null or l_info.h is null then
      Error('Incorrect PNG file (missing IHDR): ' || pFile);
    end if;
    if dbms_lob.getlength(l_info.data) = 0 then
      Error('Incorrect PNG file (missing IDAT): ' || pFile);
    end if;
    if l_info.cs = 'Indexed' and l_info.pal is null then
      Error('Missing palette in ' || pFile);
    end if;

    l_info.f := 'FlateDecode';
    return l_info;
  exception
    when others then
      if l_info.data is not null and dbms_lob.istemporary(l_info.data) = 1 then
        dbms_lob.freetemporary(l_info.data);
      end if;
      if l_info.pal is not null and dbms_lob.istemporary(l_info.pal) = 1 then
        dbms_lob.freetemporary(l_info.pal);
      end if;
  ...
  ------------------------------------------------------------------------------
  -- Parse JPEG dimensions/color space. The original JPEG stream is embedded
  -- directly in the PDF with /DCTDecode; no image conversion is performed.
  ------------------------------------------------------------------------------
  function parse_jpeg(p_blob blob) return recImage is
    l_info       recImage;
    l_pos        pls_integer := 3; -- after FFD8
    l_blob_len   pls_integer := dbms_lob.getlength(p_blob);
    l_byte       pls_integer;
    ...
    l_components pls_integer;
  begin
    while l_pos <= l_blob_len loop
      -- Find marker prefix FF.
      loop
        exit when l_pos > l_blob_len;
        l_byte := raw_u8(dbms_lob.substr(p_blob, 1, l_pos));
        l_pos := l_pos + 1;
        exit when l_byte = 255;
      end loop;

      exit when l_pos > l_blob_len;

      -- Skip fill FF bytes and obtain marker code.
      loop
        l_marker := raw_u8(dbms_lob.substr(p_blob, 1, l_pos));
        l_pos := l_pos + 1;
        exit when l_marker != 255;
        exit when l_pos > l_blob_len;
      end loop;

      -- Standalone markers.
      if l_marker = 216 or l_marker = 217 or l_marker = 1
         or (l_marker between 208 and 215) then
        if l_marker = 217 then
          exit;
        end if;
      else
        if l_pos + 1 > l_blob_len then
          Error('Invalid JPEG segment: ' || pFile);
        end if;

        l_seg_len := raw_u16(dbms_lob.substr(p_blob, 2, l_pos));
        if l_seg_len < 2 or l_pos + l_seg_len - 1 > l_blob_len then
          Error('Invalid JPEG segment length: ' || pFile);
        end if;

        -- SOF markers containing precision, height, width, components.
        if l_marker in (192,193,194,195,197,198,199,201,202,203,205,206,207) then
          if l_seg_len < 8 then
            Error('Invalid JPEG SOF segment: ' || pFile);
          end if;

          l_sof := dbms_lob.substr(p_blob, 6, l_pos + 2);
          l_precision  := raw_u8(l_sof, 1);
          l_info.h     := raw_u16(utl_raw.substr(l_sof, 2, 2));
          l_info.w     := raw_u16(utl_raw.substr(l_sof, 4, 2));
          l_components := raw_u8(l_sof, 6);
          l_info.bpc   := l_precision;

          if l_components = 1 then
            l_info.cs := 'DeviceGray';
          elsif l_components = 3 then
            l_info.cs := 'DeviceRGB';
          elsif l_components = 4 then
            l_info.cs := 'DeviceCMYK';
          else
            Error('Unsupported JPEG color components (' || l_components || '): ' || pFile);
          end if;

          exit;
        end if;

        -- Segment length includes the two length bytes already at l_pos.
        l_pos := l_pos + l_seg_len;
      end if;
    end loop;

    if l_info.w is null or l_info.h is null then
      Error('Unable to read JPEG dimensions: ' || pFile);
    end if;

    dbms_lob.createtemporary(l_info.data, true);
    dbms_lob.copy(l_info.data, p_blob, l_blob_len, 1, 1);
    l_info.f := 'DCTDecode';
    l_info.parms := null;
    return l_info;
  exception
...
begin
  myblob := getImageFromUrl(pFile);

  if myblob is null or dbms_lob.getlength(myblob) < 2 then
    Error('Invalid or empty image: ' || pFile);
  end if;

  if dbms_lob.getlength(myblob) >= 8 then
    l_sig8 := dbms_lob.substr(myblob, 8, 1);
  end if;
  l_sig2 := dbms_lob.substr(myblob, 2, 1);

  if l_sig8 = hextoraw('89504E470D0A1A0A') then
    myImgInfo := parse_png(myblob);
  elsif l_sig2 = hextoraw('FFD8') then
    myImgInfo := parse_jpeg(myblob);
  else
    Error('Unsupported image format (supported: PNG, JPEG): ' || pFile);
  end if;
    ...
    if gb_mode_debug then
	  ...
	  for i in pdfDoc.first..pdfDoc.last loop
	    if i is not null then
	      print(replace(replace(pdfDoc(i),'>','null;'),'<','null;'));
		end if;
	  end loop;
	  print('</pre>');
	end if;
...
begin
	-- Output a string
	s:='BT '|| tochar(px*k,2) ||' '|| tochar((h-py)*k,2) ||' Td '||p_pdf_text(ptxt)||' Tj ET';
...
procedure AddFont (family varchar2, style varchar2 default '',filename varchar2 default '') is
  ...
  myType varchar2(256); -- ????????? Cette variable est peut-etre globale ????????????
  -- tabNull tv4000;
begin
...
----------------------------------------------------------------------------------------
procedure SetFontFx(pfamily varchar2,pstyle varchar2 default '',psize number default 0) is
myfamily word := pfamily;
mystyle  word := pstyle;
mysize   number := psize;
FontCount number := 0;
...
begin
	...
	loop
	    wdth := 0;
		c := substr(pstr,i,1);
		wdth := p_char_width(c, charSetWidth);
		w:= w + wdth;
	end loop;
...
begin
  null;
	...
	if ptxt is not null then
		...
		-- AL32UTF8-safe: write WinAnsi bytes as a PDF hexadecimal string.
    myS := myS || 'BT '||tochar((x+myDX)*myK,2)||' '||tochar((h-(y+.5*ph+.3*fontsize))*myK,2)||' Td '||p_pdf_text(ptxt)||' Tj ET';
		if(underline) then
...
procedure MultiCell
  ...
  -- 03/12/2012 aumento dim max da txt [varchar2(2000)] a 4000
  -- 06/11/2023 aumento dim max da txt 4k a 10k
  myS varchar2(32767 byte); -- AL32UTF8: avoid 10K intermediate buffer overflow
  myNB number;
  wmax number;
  myB txt;
  myB2 txt;
...
begin
	...
	loop
		...
		if(carac = CHR(10)) then
			...
			-- si on passe la on continue a la prochaine iteration de la boucle 
		...
		if (not lb_skip) then 
			...
			l := l + p_char_width(carac, charSetWidth);
			if( l > wmax) then
  ...
  -- Loop over all characters
    ...
    -- Explicit line break
      ...
      myW := w - rMargin - x;
         ...
         lsep := lsep + p_char_width(c, charSetWidth);
			end if; 
			l := l + p_char_width(c, charSetWidth);
			if l > myWmax then
				-- Automatic line break
...
----------------------------------------------------------------------------------------
function return_output_blob(pname varchar2 default null) return blob is
   --myName word := pname;
   --myDest word := pdest;
...
--  pdfDoc.delete; -- nonno no non funge -- in realta non basta---
--  pages.delete; --non basta neppure questo
  --dbms_lob.freetemporary(v_blob);
  --dbms_lob.freetemporary(v_doc);

  return v_doc;

end return_output_blob;
----------------------------------------------------------------------------------------

-- call this when you want to create more than one pdf
   ...
   dbms_lob.createtemporary(v_doc, false, dbms_lob.session);
	 ...
	 if (myDest = 'I') then 
      ...
      wpg_docload.download_file(v_doc);
			...
			-- Return as a string
			for i in pdfDoc.first..pdfDoc.last loop
			  htp.prn(replace(replace(replace(pdfDoc(i),'<', 'null;'),'>','null;'),chr(10),'<br/>'));
			end loop;
		else
...
/* TEST HELLOWORD */
	Cell(0,1.2,'(Helloworld avec des parentheses !)',0,1,'C');

   
/*
 -- Test image
   print('Test de chargement d''une image a partir d''une url...<br>');
 ...
 Image(img,1, 1, 10);
 Cell(0,1.2,'Validation des competences du B2i',0,1,'C');
 
/*
  AddPage();
  Cell(0,1.2,'Validation des competences du B2i page 2',0,1,'C');
...
begin
  ...
  --- Aggiunte 01/12/2011  
  procedure Ellipse(x in number, y in number, rx in number, ry in number, style in char default 'D') is
    op varchar2(2);
    lx number;
    ly number;    
  begin
    ...
    p_out(tochar((x+rx)*k, 2) || ' ' || tochar((h-y)*k, 2) || ' m ' || 
          tochar((x+rx)*k, 2) || ' ' || tochar((h-(y-ly))*k, 2) || ' ' || 
          tochar((x+lx)*k, 2) || ' ' || tochar((h-(y-ry))*k, 2) || ' ' || 
          tochar(x*k, 2) || ' ' || tochar((h-(y-ry))*k, 2) || ' c');
    p_out(tochar((x-lx)*k, 2) || ' ' || tochar((h-(y-ry))*k, 2) || ' ' || 
          tochar((x-rx)*k, 2) || ' ' || tochar((h-(y-ly))*k, 2) || ' ' || 
          tochar((x-rx)*k, 2) || ' ' || tochar((h-y)*k, 2) || ' c');
    p_out(tochar((x-rx)*k, 2) || ' ' || tochar((h-(y+ly))*k, 2) || ' ' || 
          tochar((x-lx)*k, 2) || ' ' || tochar((h-(y+ry))*k, 2) || ' ' || 
          tochar(x*k, 2) || ' ' || tochar((h-(y+ry))*k, 2) || ' c');
    p_out(tochar((x+lx)*k, 2) || ' ' || tochar((h-(y+ry))*k, 2) || ' ' || 
          tochar((x+rx)*k, 2) || ' ' || tochar((h-(y+ly))*k, 2) || ' ' ||
          tochar((x+rx)*k, 2) || ' ' || tochar((h-y)*k, 2) || ' c ' || op);
  end Ellipse;
  
  procedure Circle(x in number, y in number, r in number, style in char default 'D') is
  begin
  ...
  begin
    hp := hPt;
    p_out(tochar(x1*k, 2) || ' ' || tochar((hp-y1)*k, 2) || ' ' || 
          tochar(x2*k, 2) || ' ' || tochar((hp-y2)*k, 2) || ' ' || 
          tochar(x3*k, 2) || ' ' || tochar((hp-y3)*k, 2) || ' c ');
  end Arc;
  ...
  procedure RoundedRect(x in number,
    ...
    begin
      ...
      myArc := 4/3 * (sqrt(2) - 1);
      p_out(tochar((x+rTopLeft)*k, 2) || ' ' || tochar((hp-y)*k, 2) || ' m');
      
      xc := x + w - rTopRight;
      yc := y + rTopRight;
      p_out(tochar(xc*k, 2) || ' ' || tochar((hp-y)*k, 2) || ' l');
      Arc(xc + rTopRight*MyArc, yc - rTopRight, xc + rTopRight, yc - rTopRight*MyArc, xc + rTopRight, yc);
      
      xc := x + w - rBottomRight;
      yc := y + h - rBottomRight;
      p_out(tochar((x+w)*k, 2) || ' ' || tochar((hp-yc)*k, 2) || ' l');
      Arc(xc + rBottomRight, yc + rBottomRight*MyArc, xc + rBottomRight*MyArc, yc + rBottomRight, xc, yc + rBottomRight);
     
      xc := x + rBottomLeft;
      yc := y + h - rBottomLeft;
      p_out(tochar(xc*k, 2) || ' ' || tochar((hp-(y+h))*k, 2) || ' l');
      Arc(xc - rBottomLeft*MyArc, yc + rBottomLeft, xc - rBottomLeft, yc + rBottomLeft*MyArc, xc - rBottomLeft, yc);
      
      xc := x + rTopLeft;
      yc := y + rTopLeft;
      p_out(tochar((x)*k, 2) || ' ' || tochar((hp-yc)*k, 2) || ' l');
      Arc(xc - rTopLeft, yc - rTopLeft*MyArc, xc - rTopLeft*MyArc, yc - rTopLeft, xc, yc - rTopLeft);
  ...
  procedure MultiCellWithBullet(w in number,
                                ...
                                border in varchar2 default 0,
                                align in varchar2 default 'J',
                                fill in number default 0) is
     bulletWidth  number;  
     oldX         number;                         
  begin
      -- Get bullet width including margins
      bulletWidth := GetStringWidth(blt) + cMargin*2;

      -- Save x
      oldX := x;

      -- Output bullet
      Cell(bulletWidth, h, blt, 0, '', fill);

      -- Output text
      MultiCell(w - blt_margin, h, txt, border, align, fill);

      -- Restore x
      x := oldX;
  end MultiCellWithBullet;
  
  procedure TextWithDirection(x in number, y in number, p_txt in varchar2, p_direction in varchar2 default 'R') is
    textColorString varchar2(100) := '';
    Q varchar2(2) := '';
  begin
    if ColorFlag then
      textColorString := 'q ' || TextColor || ' ';
      Q := ' Q';
    end if;
    
    if p_direction = 'R' then
      p_out(textColorString || ' BT 1 0 0 1 ' || tochar(x*k, 2) || ' ' || tochar((h-y)*k, 2) ||
        ' Tm ' || p_pdf_text(p_txt) || ' Tj ET' || Q);
    elsif p_direction = 'L' then
      p_out(textColorString || ' BT -1 0 0 -1 ' || tochar(x*k, 2) || ' ' || tochar((h-y)*k, 2) ||
      ' Tm ' || p_pdf_text(p_txt) || ' Tj ET' || Q);
    elsif p_direction = 'U' then
      p_out(textColorString || ' BT 0 1 -1 0 ' || tochar(x*k, 2) || ' ' || tochar((h-y)*k, 2) ||
      ' Tm ' || p_pdf_text(p_txt) || ' Tj ET' || Q);
    elsif p_direction = 'D' then
      p_out(textColorString || ' BT 0 -1 1 0 ' || tochar(x*k, 2) || ' ' || tochar((h-y)*k, 2) ||
      ' Tm ' || p_pdf_text(p_txt) || ' Tj ET' || Q);
    else
      p_out(textColorString || ' BT ' || tochar(x*k, 2) || ' ' || tochar((h-y)*k, 2) ||
        ' Td ' || p_pdf_text(p_txt) || ' Tj ET' || Q);
    end if;
    
  end TextWithDirection;

  procedure TextWithRotation(x in number, y in number, p_txt in varchar2, p_textAngle in number, p_fontAngle in number default 0) is
    fontAngle number;
    textAngle number;
    ...
    Q varchar2(2) := '';
  begin
    fontAngle := (p_fontAngle + 90 + p_textAngle)* PI/180;
    textAngle := p_textAngle * PI/180;

    txt_dx  := cos(textAngle);
    txt_dy  := sin(textAngle);
    ...
    p_out(textColorString || 'BT ' || tochar(txt_dx, 2) || ' ' || tochar(txt_dy, 2) || ' ' ||
      tochar(font_dx, 2) || ' ' || tochar(font_dy, 2) || ' ' || tochar(x*k, 2) || ' ' ||
      tochar((h-y)*k, 2) || ' Tm ' || p_pdf_text(p_txt) || ' Tj ET' || Q);
    
  end TextWithRotation;
  
  procedure ClippingRect(px in number, py in number, pw in number, ph in number, p_outline in boolean default false) is
    op char(1);
  begin
    if (p_outline) then
      op := 'S';
    ...
    p_out('q ' || tochar(px * k, 2) || ' ' || tochar((h - py) * k, 2) || ' ' ||
      tochar(pw * k, 2) || ' ' || tochar(-ph * k, 2) || ' re W ' || op);
  end ClippingRect;
 
  procedure UnsetClipping is
  begin
    p_out('Q');
  end UnsetClipping;

  /*procedure Polygon(points t_points, style varchar2 default 'D') is
    op            varchar2(2);
    pointsString  varchar2(1000);
  begin
    null;
    ...
    for i in 1 .. points.count loop
      pointsString := pointsString || tochar(points(i)*k, 2) || ' ' || tochar((h-points(i+1))*k, 2);
      if i = 1 then
        pointsString := pointsString || ' m ';
      else
        pointsString := pointsString || ' l ';
      end if;
    end loop;
    
    p_out(pointsString || op);
  end Polygon;*/
END PL_FPDF;
/
